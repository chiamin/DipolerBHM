...
template <typename T>
T read_value (ifstream& ifs, const string& key, const string& delimiter="=")
{
    ifs.clear();
    ifs.seekg(0);
    ...
    while (getline(ifs, line))
    {
        ...
        {
            // Read data
            vector<string> re = split_str<string> (line);
            if (re.size() == 3 and re[0] == key and re[1] == delimiter)
            {
                return convert_type<T> (re[2]);
            }
        }
    }
    cout << "ReadInput.h: read_value: " << key << " not found" << endl;
    throw;
}
